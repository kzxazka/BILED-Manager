...
<div class="space-y-6">
    ...
    <div>
        ...
        <h2 class="text-xl font-bold text-zinc-100">Tambah Produk Baru</h2>
        <p class="text-xs text-zinc-400">Masukkan rincian produk atau bahan retrofit baru</p>
    </div>
    ...
    <div class="bg-zinc-900/90 rounded-2xl p-5 border border-zinc-800 shadow-sm">
        <form action="{{ route('products.store') }}" method="POST" class="space-y-4">
            ...
            <div class="space-y-1">
                ...
                <input type="text" name="name" id="name" placeholder="contoh: Projector BILED 3.0 inch" value="{{ old('name') }}" required
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="space-y-1">
                    ...
                    <input type="number" name="hpp_price" id="hpp_price" placeholder="contoh: 450000" value="{{ old('hpp_price') }}" required min="0"
                           class="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-3 py-3 text-xs text-zinc-200 placeholder-zinc-700 focus:outline-none focus:border-[#00F0FF]/50 transition-colors">
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <input type="number" name="sell_price" id="sell_price" placeholder="contoh: 750000" value="{{ old('sell_price') }}" required min="0"
                           class="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-3 py-3 text-xs text-zinc-200 placeholder-zinc-700 focus:outline-none focus:border-[#00F0FF]/50 transition-colors">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <!-- Stock -->
                <div class="space-y-1">
                    <label for="stock" class="text-[10px] font-semibold text-zinc-400 uppercase tracking-wide">Stok Awal</label>
                    <input type="number" name="stock" id="stock" placeholder="contoh: 10" value="{{ old('stock', 0) }}" required min="0"
                           class="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-3 py-3 text-xs text-zinc-200 placeholder-zinc-700 focus:outline-none focus:border-[#00F0FF]/50 transition-colors">
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <input type="number" name="min_stock" id="min_stock" placeholder="contoh: 2" value="{{ old('min_stock', 2) }}" required min="0"
                           class="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-3 py-3 text-xs text-zinc-200 placeholder-zinc-700 focus:outline-none focus:border-[#00F0FF]/50 transition-colors">
                </div>
            </div>
